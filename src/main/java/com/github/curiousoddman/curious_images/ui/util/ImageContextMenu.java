...
public class ImageContextMenu {
    ...
    public ContextMenu show(PhotoRecord photo,
                            Parent parent,
                            ContextMenuEvent e
    ) {
        if (photo == null) {
        ...
        rotateCw.setOnAction(ev -> rotateCurrentPhoto(photo, PhotoRotationService.ROTATE_CW));
        ...
        rotateCcw.setOnAction(ev -> rotateCurrentPhoto(photo, PhotoRotationService.ROTATE_CCW));

        MenuItem rotate180 = new MenuItem("Rotate 180°", new FontIcon(ARROW_REPEAT));
        rotate180.setOnAction(ev -> rotateCurrentPhoto(photo, PhotoRotationService.ROTATE_180));

        MenuItem reveal = new MenuItem("Reveal in Explorer", new FontIcon(FOLDER_SYMLINK));
        reveal.setOnAction(ev -> ExplorerUtils.revealInExplorer(photo.getAbsolutePath()));
